...
def build_directives(crs: Path, profile: str) -> tuple[list[str], dict]:
    rules_dir = crs / "rules"
...
def write_envoy_yaml(plugin_json: str, out: Path, vm_id: str) -> None:
...
def main() -> int:
    ...
    try:
        directives, stats = build_directives(crs, args.profile)
    ...
    if args.out_envoy:
        write_envoy_yaml(plugin_json, args.out_envoy, args.vm_id)
