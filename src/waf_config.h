...
//   @crs-data/*.data   — CRS phrase lists for @pmFromFile (full catalog only;
//                        path-b uses plugin JSON data_files at runtime)
// path-b catalog: helpers only. CRS rule confs are NOT embedded. Use structured
// SecRule CRs / inline SecLang. Include @crs-setup-conf / @owasp_crs* fail closed.
...
// Max per-file inflated size: 2 MiB; total inflated: 2 MiB.
