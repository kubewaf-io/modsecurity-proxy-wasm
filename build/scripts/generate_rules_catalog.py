#!/usr/bin/env python3
"""Embed OWASP CRS + virtual @-includes for the modsec-wasm Envoy plugin."""
...
def patch_crs_setup(example: str) -> str:
    ...
    body = "\n".join(lines)
    header = (
        "# Auto-generated @crs-setup-conf for modsec-wasm (from crs-setup.conf.example)\n"
...
def emit_catalog(assets: list[tuple[str, str]], out_cc: Path, out_h: Path) -> None:
    ...
    body.append("}")
    body.append("")
    body.append("}  // namespace modsec_wasm_rules")
    out_cc.write_text("\n".join(body) + "\n", encoding="utf-8")
